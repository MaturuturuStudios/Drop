...
public class KeyEditorWindow : EditorWindow {
    ...
    private void DrawListOfKeys() {
        GUILayout.Space(EditorGUIUtility.singleLineHeight);
        GUILayout.Label("List of key used in the game", EditorStyles.boldLabel);
        CustomGUI.Splitter(Color.black);

        scrollPos = EditorGUILayout.BeginScrollView(scrollPos,
            GUILayout.Width(position.width), GUILayout.Height(300));

        foreach (FieldInfo field in typeof(PlayerDataStoreKeys).GetFields()) {
            ...
            if (PlayerPrefs.HasKey(key)) {
                ...
                PlayerPrefsValueType typeKey = GetType(key);
                GUILayout.Label(GetValue(key, typeKey));
            }
            EditorGUILayout.EndHorizontal();
        }

        foreach (FieldInfo field in typeof(OptionsKey).GetFields()) {
            EditorGUILayout.BeginHorizontal();
            string key = field.GetValue(null).ToString();
            GUILayout.Label(key + ": ", EditorStyles.boldLabel);
