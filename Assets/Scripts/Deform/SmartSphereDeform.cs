...
public class SmartSphereDeform : MonoBehaviour {
	...
	/// <summary>
	/// The distance nearby vertices will be applied by the chamf
	/// deformation.
	/// </summary>
	...
	private void DeformVertices(int rayIndex, Vector3 deformation, ref Vector3[] vertexToModify) {
        ...
        for (int i = 0; i < vertexToModify.Length; i++) {
			...
			// Calculates the chamf. Filters the vertex by its distance to the deformation point
            ...
            if (distanceProjection.sqrMagnitude <= (chamfPenetration * deformation).sqrMagnitude) {
				...
				offset *= chamfScale / _transform.lossyScale.x;
