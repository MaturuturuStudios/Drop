...
public class CannonShoot : ActionPerformer
{
	...
	public void setvisibility()
	{
		...
		for (int i = 0; i < numOfTrajectoryPoints - 1 && !colisiondetected; i++)
		{
			...
			if ((Physics.Raycast(trajectoryPoints[i].transform.position, fwd, dis, layerMask)))
			{
	...
	public void OnDrawGizmos()
	{
		if (!Application.isPlaying) {
			...
			for (int i = 0; i < numOfTrajectoryPoints; i++) {
				...
				Vector3 posi = transform.position + new Vector3(dx, dy, 0);
				Vector3 d = posi - lastPoint;

				if (Physics.Raycast(lastPoint, d, out hitpoint, d.magnitude, layerMask)) {
					Gizmos.DrawLine(lastPoint, hitpoint.point);
					break;
				}
				else
				{
					Gizmos.DrawLine(lastPoint, posi);
					lastPoint = posi;
				}
			}
		}
